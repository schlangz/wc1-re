...
{
#ifdef SDL_PORT
    /* The packed format reserves one width entry for every byte value. */
    return pCurrentTextContext->font[4 + (unsigned char)i];
#else
    ...
    for (;;) {
        ...
        if (lineWidth < right) {
            for (;;) {
                ...
                lineWidth +=
#ifdef SDL_PORT
                    pCurrentTextContext->font[4 + (unsigned char)value];
#else
